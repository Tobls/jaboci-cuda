...
__global__ void jacobi(const float * __restrict__ A, const float * __restrict__ b, float * __restrict__ x, float * __restrict__ xNew, int N) {
    ...
    for (int j = 0; j < N; ++j) {
        if (j != i) {
            sum += sharedA[i][j] * sharedX[j];
        }
    }
    xNew[i] = (b[i] - sum) / sharedA[i][i];
}
